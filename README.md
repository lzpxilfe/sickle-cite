...
## 1.8.0 Safari 논문 다운로드 강화

- **서지 추출 보완:** 기존 사이트별 파서에 citation 메타 태그, Dublin Core, JSON-LD, 페이지에 포함된 단일 RIS/BibTeX 레코드를 병합합니다. AccessON의 중복 영문 이름을 제외하고 한글 서지를 우선하며, KoreaScience·dCollection·RISS 모바일 상세정보를 보완합니다. 목록의 PDF 버튼은 인접한 단일 논문 행의 정보를 사용합니다.
- **새 탭·뷰어 연결:** 실제 클릭, 원문 URL/논문 ID, opener 탭 관계로 메타데이터를 연결합니다. 임시 컨텍스트는 세션 저장소(미지원 시 로컬)에 30분·60개 한도로 보관하며, 서비스 워커가 재시작되어도 복원합니다. 브라우저 재시작 시 초기화합니다. 출처가 모호하면 최근 논문이라는 이유만으로 이름을 붙이지 않습니다.
- **Safari 저장 경로:** 페이지에서 PDF를 받아 인용 파일명으로 저장합니다. 중간 HTML의 PDF/iframe을 최대 3단계·6요청까지 탐색하고 PDF 헤더를 확인합니다. 페이지의 교차 출처 요청이 실패하면 확장 자체 저장 화면으로 연결합니다. 저장 화면에서는 PDF 준비 후 사용자가 저장 버튼을 눌러 최종 저장을 시작합니다.
- **뷰어 API:** dCollection StreamDocs 및 PDF.js 뷰어에서는 팝업의 `현재 뷰어의 PDF`를 통해 파일명을 지정합니다. 해당 뷰어의 다운로드 API/데이터 API와 `scripting` MAIN 실행이 지원되어야 합니다. 사이트의 이용 권한 검사는 그대로 적용됩니다.
- **기본 PDF 화면:** 상세페이지에서 연결된 PDF 탭은 팝업에서 컨텍스트를 복원하고 별도 저장 화면을 이용할 수 있습니다. 다운로드가 이미 끝난 로컬 파일을 사후에 바꾸는 기능은 아닙니다.
- **파일명·오인 방지:** UTF-8 235바이트 상한, 한글 NFC 정규화, 중복 클릭 방지, Blob URL 60초 유지, 인용·프로그램 설치·업로드·비PDF·일반 안내 링크 제외, 동일 URL 중복 제거를 적용합니다. KCI `fncDown`의 인자 1개/2개 형태를 모두 처리합니다.

### 범위와 권한

기존 DB 외에 `koreascience.kr`, 코리아스칼라, AccessON, KoreaMed/Synapse, 국회도서관, 국립중앙도서관, AURIC, KMbase, ScholarWorks와 대학 기관 저장소, Google Scholar를 인식합니다. `ac.kr`/`edu` 대학 프록시의 점·하이픈 호스트를 처리합니다. 그 밖의 학회 홈페이지도 논문용 citation/DC/ScholarlyArticle 표식이 있으면 공통 추출 경로를 사용합니다.

대학별 프록시와 학회 도메인을 열거할 수 없어 HTTP/HTTPS 웹사이트 권한을 요청합니다. 일반 웹페이지에서는 학술 표식 여부만 확인하고 콘텐츠 리스너·정보 수집을 시작하지 않습니다. Safari에서 해당 사이트 접근 권한이 허용되어 있어야 작동합니다. DOI 외부 조회나 외부 분석 서비스는 사용하지 않습니다.

### 검증과 제한

`npm ci && npm test`로 테스트합니다. 공개 상세페이지 12개에서 전체 콘텐츠 스크립트의 서지 추출과 다운로드 후보를 점검했고, AccessON·KoreaMed Synapse의 PDF 응답, KCI·KoreaScience의 정상 상세페이지 세션에서 PDF 헤더를 확인했습니다. 자동 테스트는 DOM·네트워크·확장 API 모의 환경이며, Safari에서 기관 로그인 후 실제 파일 저장까지 모든 사이트를 검증한 것은 아닙니다.

현재 Xcode 변환기는 `downloads` 권한 미지원 경고를 출력하므로 Safari에서는 콘텐츠 스크립트/뷰어 API/별도 저장 화면을 사용합니다. 기관인증·결제·DRM·관내 전용·이미지 전용 원문이나 다운로드 기능이 없는 서지 전용 자료는 자동 저장을 보장하지 않습니다. 요청 실패 시 원래 사이트 흐름 또는 복사한 인용 파일명을 이용할 수 있습니다. 테스트 자료·접근 범위는 `SUPPORT.md`를 참고하세요.

참고 설계: [paper-rename-extension](https://github.com/lzpxilfe/paper-rename-extension). 이 포크의 기존 코드와 Safari 실행 환경에 맞춰 구현했습니다.
...
- 추가 서비스와 대학 프록시·학회 사이트의 조건부 지원 범위는 위 1.8.0 설명 및 SUPPORT.md를 참고하세요.
